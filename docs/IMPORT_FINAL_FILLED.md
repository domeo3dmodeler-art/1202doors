...
## После импорта: сброс кэша конфигуратора

Ответ GET `/api/catalog/doors/complete-data` кэшируется на 30 минут. Чтобы конфигуратор сразу показывал новые данные:

- Вызвать **DELETE** `/api/catalog/doors/complete-data` (с авторизацией), **или**
- Открыть конфигуратор с параметром **?refresh=1** (например `/doors?refresh=1`).
...
3. **Проверка на UI** (вручную после `npm run dev`):
   - Открыть страницу конфигуратора: **`/doors`**.
...
- Конфигуратор дверей (`/doors`) берёт модели и цены из **Product**, фото и цвета покрытий из **PropertyPhoto**.
