...
Документ объединяет результаты аудита и фиксирует один предсказуемый цикл работы с приложением на ВМ **178.154.244.83** (каталог `~/domeo-app`).
...
3. Открыть http://178.154.244.83 (или ваш IP). При правках кода — повторить п.2 или только `.\scripts\restart-vm-app.ps1` после ручного обновления файлов.
...
3. Открыть http://178.154.244.83. Локальные правки подтягивать через `.\scripts\push-one-file-to-vm.ps1 app\...`.
...
- `1002DOORS_STAGING_HOST` — например `ubuntu@178.154.244.83`.
- `1002DOORS_STAGING_REMOTE_PATH` — например `~/domeo-app`.

По умолчанию скрипты используют `ubuntu@178.154.244.83` и `~/domeo-app`.
