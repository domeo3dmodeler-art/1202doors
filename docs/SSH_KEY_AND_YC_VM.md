...
Для выкладки проекта на ВМ Yandex Cloud с IP **158.160.74.180** используйте тот же подход, что и для staging (178.154.244.83): ключ не хранить в репозитории, подключаться по ключу, в скриптах — путь из переменной окружения.
...
$env:1002DOORS_STAGING_HOST = "ubuntu@178.154.244.83"       # или petr@178.154.244.83 — как на ВМ
```
