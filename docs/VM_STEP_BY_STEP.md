...
**Рабочая машина (основная ВМ):** **178.154.244.83**. Все скрипты по умолчанию используют хост `ubuntu@178.154.244.83` и каталог `~/domeo-app`. Тестовая ВМ: 178.154.244.83 — см. docs/VM_HOSTS.md. Если у вас в окружении задана другая ВМ (`1002DOORS_STAGING_HOST`), чтобы вернуться к этой, в PowerShell выполните:  
...
1. Убедитесь, что у вас есть приватный ключ для доступа к ВМ (например `C:\Users\petr2\.ssh\ssh-key-1773410153319\ssh-key-1773410153319`).
2. В PowerShell (для текущей сессии) задайте переменную, если путь другой:
   ...
   ```
   Если ключ уже по умолчанию в скриптах (`C:\Users\petr2\.ssh\ssh-key-1773410153319\ssh-key-1773410153319`), переменную можно не ставить.

**Проверка:** файл существует:
```powershell
Test-Path $env:1002DOORS_SSH_KEY
# или
Test-Path "C:\Users\petr2\.ssh\ssh-key-1773410153319\ssh-key-1773410153319"
```
Должно быть `True`.
...
1. Задайте переменные, если ваша ВМ не `178.154.244.83` и каталог не `~/domeo-app`:
   ```powershell
   $env:1002DOORS_STAGING_HOST = "ubuntu@IP_ВАШЕЙ_ВМ"
   $env:1002DOORS_STAGING_REMOTE_PATH = "~/domeo-app"
   ```
2. По умолчанию скрипты используют `ubuntu@178.154.244.83` и `~/domeo-app` — тогда переменные не нужны.

**Проверка:** запомните, что будете использовать:
- Хост: `$env:1002DOORS_STAGING_HOST` или `ubuntu@178.154.244.83`
...
1. Из корня проекта выполните (подставьте свой ключ и хост при необходимости):
   ```powershell
   ssh -i "C:\Users\petr2\.ssh\ssh-key-1773410153319\ssh-key-1773410153319" -o StrictHostKeyChecking=no ubuntu@178.154.244.83 "echo OK"
   ```
...
1. Откройте в браузере: **http://178.154.244.83** (или IP вашей ВМ).
...
**Проверка:** в браузере http://178.154.244.83 открывается без 502.
