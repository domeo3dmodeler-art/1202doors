...
**Рабочая машина (основная ВМ):** **89.169.181.191**. Все скрипты по умолчанию используют хост `ubuntu@89.169.181.191` и каталог `~/domeo-app`. Тестовая ВМ: 130.193.62.116 — см. docs/VM_HOSTS.md. Если у вас в окружении задана другая ВМ (`1002DOORS_STAGING_HOST`), чтобы вернуться к этой, в PowerShell выполните:  
