...
6. Проверка: http://178.154.244.83:3000/doors — модели должны загружаться, каталог работать со старым набором (~2204 товара).
...
2. **Через консоль Yandex Cloud (надёжный способ):**  
   [Консоль Yandex Cloud](https://console.yandex.cloud/) → Compute Cloud → Виртуальные машины → выберите ВМ с IP 178.154.244.83 → **«Подключиться»** или меню **Действия** → **Перезапустить** (или Остановить → Запустить). Так перезагрузка не зависит от SSH.
