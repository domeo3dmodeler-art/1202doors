# ВМ 178.154.244.83 — доступ только с вашего IP и снижение трафика
...
1. Откройте [консоль Yandex Cloud](https://console.yandex.cloud/) → **VPC** → **Группы безопасности** (или **Compute Cloud** → ВМ **178.154.244.83** → вкладка **Сеть** → группа безопасности).
...
- [ ] В группе безопасности ВМ 178.154.244.83: входящие только **22** и **80** с источника **мой_IP/32**.
- [ ] Сохранил группу, подождал 1–2 минуты.
- [ ] Подключился по SSH с ПК, выполнил `.\scripts\apply-vm-traffic-fix.ps1 -MyIp "мой_IP"`.
- [ ] Проверил сайт: http://178.154.244.83 (с того же ПК).
