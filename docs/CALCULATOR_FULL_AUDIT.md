...
## 7. Устаревшие/неиспользуемые в калькуляторе (legacy)

Следующее **не используется** в логике калькулятора — это старые данные/поля, оставшиеся в коде или БД:

1. **Тип конструкции** (`selection.type`, свойство товара «Тип конструкции»): в API price/doors есть проверка `typeMatch`, но в калькуляторе type **нигде не передаётся и не выбирается**. В БД у дверей поле часто не заполнено. Подбор цены идёт без типа конструкции. Оставлять как есть или убрать typeMatch из API при желании упростить код.
2. **Domeo_Цвет в товаре:** в товаре поле может быть пустым; в фильтре цены допускается `color == null`. Фактический цвет для пользователя берётся из выбора в конфигураторе (покрытие + цвет), а не из свойства товара. То есть «Domeo_Цвет в товаре» в расчёте не используется как источник истины.
3. **Звукоизоляция (Стандартное/Хорошее/Отличное):** блок «Звукоизоляция» в спецификации и state `filling` (standard/good/excellent) — **старые данные**, на расчёт цены и на подбор товара **не влияют**. В калькуляторе для цены и фильтров используется только **наполнение из каталога** (selectedFilling: Голд, Сильвер и т.д.). Строку «Звукоизоляция» в спецификации можно оставить как справочную или убрать при упрощении UI.
