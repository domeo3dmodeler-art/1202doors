...
2. **Применение Nginx на ВМ** — копирует `scripts/output/domeo-nginx.conf` на 89.169.181.191, `nginx -t` и `reload`.
...
1. В репозитории ключ для 89.169.181.191 описан в **docs/VM_158_160_10_126_DEPLOY.md** и **docs/VM_SSH_KEY_REFERENCE.md**. По умолчанию скрипты используют `%USERPROFILE%\.ssh\ssh-key-1771526730154\ssh-key-1771526730154`.
...
- **1002DOORS_SSH_KEY** — путь к приватному ключу. Для ВМ **89.169.181.191** в репозитории задан ключ из [docs/VM_158_160_10_126_DEPLOY.md](VM_158_160_10_126_DEPLOY.md): `%USERPROFILE%\.ssh\ssh-key-1771526730154\ssh-key-1771526730154`. Если при настройке «одна ВМ» ([DEPLOY_ODNA_VM.md](DEPLOY_ODNA_VM.md)) на сервер добавлен ключ **1002doors-vm**, задайте: `$env:1002DOORS_SSH_KEY = "$env:USERPROFILE\.ssh\1002doors-vm\id_ed25519"`.
- **1002DOORS_STAGING_HOST** — хост ВМ (по умолчанию `ubuntu@89.169.181.191`).
