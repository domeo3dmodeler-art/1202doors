...
# По умолчанию — тестовая ВМ 178.154.244.83. Для рабочей: .\scripts\set-prod-vm-env.ps1
...
$restartCmd = "sudo systemctl restart domeo-standalone 2>/dev/null || (fuser -k 3000/tcp 2>/dev/null; true; sleep 2; cd $RemotePath && mkdir -p logs && set -a && [ -f .env ] && . ./.env; set +a; export PUPPETEER_EXECUTABLE_PATH=${PUPPETEER_EXECUTABLE_PATH:-/usr/bin/chromium-browser}; NODE_ENV=production PORT=3000 HOSTNAME=0.0.0.0 nohup node server.js >> logs/server.log 2>&1 &)"
