...
// В dev не загружаем jose/rate-limiter, чтобы избежать EvalError в Edge runtime (code generation disallowed)
...
export async function middleware(request: NextRequest) {
  ...
  const { getClientIP, globalApiRateLimiter, createNextRateLimitResponse } = await import('@/lib/security/rate-limiter');
  ...
  if (pathname.startsWith('/api')) {
    ...
    if (!isUploads) {
      ...
      if (!globalApiRateLimiter.isAllowed(clientIP)) {
...
export const config = {
  ...
  // Edge runtime: разрешить динамический код в зависимостях, иначе EvalError при загрузке
