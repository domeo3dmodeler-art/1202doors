...
describe('roundUpTo10', () => {
  it('округление вверх до 10 руб', () => {
    expect(roundUpTo10(0)).toBe(0);
    expect(roundUpTo10(10)).toBe(10);
    expect(roundUpTo10(11)).toBe(20);
    expect(roundUpTo10(28755)).toBe(28760);
    expect(roundUpTo10(21600)).toBe(21600);
  });
});
...
describe('calculateDoorPrice — базовая дверь', () => {
  ...
  it('11. итог = сумма breakdown округлённая вверх до 10 руб (базовая дверь)', () => {
    ...
    expect(r.total).toBe(roundUpTo10(sumBreakdown));
    expect(r.total).toBe(20000);
  });

  it('12. total округляется вверх до 10 руб', () => {
    const prodWithFloat = [
      doorProduct({
        properties_data: {
    ...
    expect(r.total).toBe(20000); // roundUpTo10(19999.7)
  });
});

describe('calculateDoorPrice — цвет по RAL/NCS (+20%)', () => {
  const products: ProductWithProps[] = [
    doorProduct({
      id: 'd-ral',
      properties_data: {
  ...
  it('добавляет 20% к стоимости двери при цвете RAL или NCS', () => {
    ...
    const doorRow = r.breakdown.find((b) => b.label === 'Дверь');
    const ralRow = r.breakdown.find((b) => b.label === 'Цвет по RAL/NCS (+20%)');
    expect(doorRow?.amount).toBe(25000);
    expect(ralRow?.amount).toBe(5000); // 20% от 25000
    expect(r.total).toBe(roundUpTo10(25000 + 5000)); // 30000
  });

  it('то же для NCS — наценка 20%', () => {
    const r = calculateDoorPrice({
      products,
      selection: {
        style: 'Современные',
    ...
    const ralRow = r.breakdown.find((b) => b.label === 'Цвет по RAL/NCS (+20%)');
    expect(ralRow?.amount).toBe(5000);
  });

  it('обычный цвет без RAL/NCS — наценки нет', () => {
    const r = calculateDoorPrice({
      products,
      selection: {
        style: 'Современные',
...
describe('calculateDoorPrice — надбавка за высоту', () => {
  ...
  it('13. надбавка за высоту 2301–2500: процент от базы, итог округляется вверх до 10 руб', () => {
    const r = calculateDoorPrice({
    ...
    expect(r.total).toBe(28750); // 28750 уже кратно 10
  });
  ...
  it('14. надбавка за высоту 2501–3000: процент от базы, итог округляется вверх до 10 руб', () => {
    const r = calculateDoorPrice({
...
describe('calculateDoorPrice — кромка, комплект, ручка, завертка, ограничитель, опции', () => {
  ...
  it('22. ограничитель добавляется по limiter_id', () => {
    ...
    expect(r.total).toBe(17450); // roundUpTo10(17000 + 450)
  });

  it('23. option_ids (наличники) не влияют на цену', () => {
    const r = calculateDoorPrice({
      products: [door],
      selection: {
        style: 'Современные',
    ...
    expect(r.breakdown.find((b) => b.label.includes('Наличник'))).toBeUndefined();
...
describe('calculateDoorPrice — связи и граничные случаи', () => {
  ...
  it('25. итоговая сумма всегда равна сумме breakdown', () => {
    ...
    const sumBreakdown = r.breakdown.reduce((s, b) => s + b.amount, 0);
    expect(r.total).toBe(roundUpTo10(sumBreakdown));
    expect(r.total).toBe(34000); // 28000+600+900+3000+1500 уже кратно 10
  });
});
